...
export async function registerUser(params: { name: string; email: string; password: string; phone?: string; address?: string }) {
  ...
  try {
    ...
    // Temporarily disabled: email verification flow (enable again when needed).
...
export async function loginUser(params: { email: string; password: string }) {
  ...
  try {
    ...
    // Temporarily disabled: block unverified users from logging in.
...
export async function resendVerificationEmail(email: string) {
  ...
  logger.info({ uid: user._id.toString() }, 'authService.resendVerification:disabled');
  return { ok: true } as const;
}
